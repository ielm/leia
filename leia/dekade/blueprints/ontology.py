...
class DEKADEAPIOntologyBlueprint(Blueprint):
    ...
    def __init__(self, app: DEKADE):
        ...
        # Read APIs
        self.add_url_rule(
            "/api/knowledge/ontology/list",
            endpoint=None,
            view_func=self.read_list,
            methods=["GET"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/filter/<substring>",
            endpoint=None,
            view_func=self.read_filter,
            methods=["GET"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>",
            endpoint=None,
            view_func=self.read_concept,
            methods=["GET"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/children/<concept>",
            endpoint=None,
            view_func=self.read_children,
            methods=["GET"],
        )

        # Write APIs
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/create",
            endpoint=None,
            view_func=self.write_create,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/parent/add",
            endpoint=None,
            view_func=self.write_parent_add,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/parent/remove",
            endpoint=None,
            view_func=self.write_parent_remove,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/definition/edit",
            endpoint=None,
            view_func=self.write_definition_edit,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/filler/add",
            endpoint=None,
            view_func=self.write_filler_add,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/filler/remove",
            endpoint=None,
            view_func=self.write_filler_remove,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/filler/block",
            endpoint=None,
            view_func=self.write_filler_block,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/knowledge/ontology/concept/<concept>/filler/unblock",
            endpoint=None,
            view_func=self.write_filler_unblock,
            methods=["POST"],
        )

    def _guess_filler(
        self, concept: Concept, property: str, filler: str
    ) -> Concept.FILLER:
    ...
    def _parse_filler(self, concept: Concept, filler: str, type: str) -> Concept.FILLER:
        ...
        raise NotImplementedError(
            "Cannot parse filler %s of type %s in %s." % (filler, type, str(concept))
        )

    def _cast_filler(
        self, concept: Concept, filler: Concept.FILLER
    ) -> Tuple[Union[str, Tuple, int, float], str]:
        ...
        raise NotImplementedError(
            "Cannot cast filler %s in %s." % (str(filler), str(concept))
        )
    ...
    def write_filler_add(self, concept: str):
        ...
        if len(data["meta"]) > 0:
            meta = dict(
                map(
                    lambda m: tuple([m[0].strip(), m[1].strip()]),
                    map(lambda m: m.strip().split("="), data["meta"].split(",")),
                )
            )
