...
class Agent(object):
    ...
    def __init__(
        self,
        memory: "Memory" = None,
        modules: List["OntoAgentModule"] = None,
        effectors: List["Effector"] = None,
    ):
        from leia.ontomem.memory import Memory
    ...
    def renderer_with_effector(
        self, effector: "Effector"
    ) -> Union[None, "OntoAgentRenderingModule"]:
        from leia.ontoagent.engine.module import OntoAgentRenderingModule

        modules = filter(
            lambda m: isinstance(m, OntoAgentRenderingModule), self.modules
        )
