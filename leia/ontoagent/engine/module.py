...
class OntoAgentModule(object):
    ...
    class NoServiceDefinedError(Exception):
        pass
    ...
    def remote_signal(self, xmr: XMR):
        url = self.service()
        data = json.dumps({"signal": xmr.name()}).encode("utf8")
...
class OntoAgentModuleBlueprint(OntoAgentBlueprint):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.add_url_rule(
            "/signal", endpoint=None, view_func=self.signal, methods=["POST"]
        )
        self.add_url_rule(
            "/heartbeat/pulse",
            endpoint=None,
    ...
    def build_payload(self) -> dict:
        ...
        payload["head"] = {
            "host": self.head_host,
            "port": self.head_port,
            "path": "http://%s:%d" % (self.head_host, self.head_port),
        }
...
class OntoAgentRenderingModule(OntoAgentModule):
    ...
    def signal_preferred_effector(
        self, xmr: XMR
    ) -> Union[None, Effector, List[Effector]]:
