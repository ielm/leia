...
class OntoAgentModuleService(Flask):
    ...
    def __init__(
        self, agent: Agent, host: str, port: int, head: Union[bool, Dict] = False
    ):
        ...
        self.add_url_rule(
            "/heartbeat/pulse",
            endpoint=None,
            view_func=self.heartbeat_pulse,
            methods=["POST"],
        )
        self.add_url_rule(
            "/heartbeat/start",
            endpoint=None,
            view_func=self.heartbeat_start,
            methods=["POST"],
        )
        self.add_url_rule(
            "/heartbeat/stop",
            endpoint=None,
            view_func=self.heartbeat_stop,
            methods=["POST"],
        )
...
class OntoAgentHeadBlueprint(OntoAgentBlueprint):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.add_url_rule(
            "/memory", endpoint=None, view_func=self.ui_memory, methods=["GET"]
        )
        self.add_url_rule(
            "/memory/<id>", endpoint=None, view_func=self.ui_memory, methods=["GET"]
        )
        self.add_url_rule(
            "/api/instance", endpoint=None, view_func=self.api_instance, methods=["GET"]
        )
        self.add_url_rule(
            "/api/script/advance",
            endpoint=None,
            view_func=self.api_script_advance,
            methods=["POST"],
        )
        self.add_url_rule(
            "/api/script/load",
            endpoint=None,
            view_func=self.api_script_load,
            methods=["POST"],
        )
    ...
    def ui_memory(self, id: str = None):
        ...
        if id is not None:
            if id.startswith("@"):
                id = id[1:]
            payload["frame"] = self.output_instance(
                self.agent.memory.episodic.instance(id)
            )
