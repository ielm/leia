...
from leia.ontoagent.engine.module import (
    OntoAgentAttentionModule,
    OntoAgentReasoningModule,
)
...
if __name__ == "__main__":
    ...
    for m in filter(
        lambda module: isinstance(module, OntoAgentReasoningModule), agent.modules
    ):
        print(m.name)

    for m in filter(
        lambda module: isinstance(module, OntoAgentAttentionModule), agent.modules
    ):
