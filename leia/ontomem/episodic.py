...
class Space(object):
    ...
    def __init__(
        self, memory: Memory, name: str, parent: "Space" = None, private: bool = False
    ):
        self.memory = memory
        self._name = name
        self._parent = parent
    ...
    def instances_of(
        self, concept: Concept, include_descendants: bool = False
    ) -> List["Instance"]:
        ...
        return list(
            filter(lambda i: i.concept.name in matches, self._instances.values())
        )

    def new_instance(
        self, concept: Union[str, Concept], instance_type: Type["Instance"] = None
    ) -> "Instance":
    ...
    def new_space(
        self, name: str = None, space_type: Type["Space"] = None, private: bool = False
    ) -> "Space":
...
class XMR(Space):
    ...
    class Status(Enum):
        ...
        INTERPRETED = (
            "INTERPRETED"  # The XMR represents input that has been interpreted
        )
    ...
    def __init__(
        ...
        super().__init__(
            ...
            (
                name
                if name is not None
                else memory.episodic._next_id_for_space(self.__class__)
            ),
            parent=parent,
            private=private,
        )
    ...
    def root(self) -> Union["Instance", None]:
        ...
        if "EVENT" in subtrees_present:
            candidates = list(
                filter(lambda s: s["subtree"] == "EVENT", root_scoring.values())
            )
        elif "OBJECT" in subtrees_present:
            candidates = list(
                filter(lambda s: s["subtree"] == "OBJECT", root_scoring.values())
            )
        elif "PROPERTY" in subtrees_present:
            candidates = list(
                filter(lambda s: s["subtree"] == "PROPERTY", root_scoring.values())
            )
        ...
        candidates = list(
            filter(lambda c: c["instance"] == lowest_instance, candidates)
        )
...
class Instance(object):
    ...
    class TooManyFillersError(Exception):
        pass
    ...
    def __init__(self, memory: Memory, concept: Union[str, Concept], index: int):
        ...
        self.concept = (
            concept
            if isinstance(concept, Concept)
            else memory.ontology.concept(concept)
        )
        ...
        self._indexes = {memory.episodic: index}
    ...
    def add_filler(
        self, slot: str, filler: "Filler.VALUE", timestamp: float = None
    ) -> "Instance":
    ...
    def set_filler(
        self,
        slot: str,
        filler: Union["Filler.VALUE", List["Filler.VALUE"]],
        timestamp: float = None,
    ) -> "Instance":
    ...
    def to_dict(self, space: Space = None) -> dict:
        ...
        return {
            ...
            "properties": dict(
                map(
                    lambda i: (i[0], list(map(lambda f: _cast_value(f.value()), i[1]))),
...
class Address(object):
    ...
    def _resolve_node(
        self, node: Dict, current: Union[Space, Instance, None]
    ) -> Union[Space, Instance, None]:
        if node["type"] == "space":
            if current is None:
    ...
    def __repr__(self):
        symbols = {"space": "&", "instance": "#", "property": "$"}

        return "/".join(
            map(lambda n: "%s%s" % (symbols[n["type"]], n["id"]), self._path)
        )
