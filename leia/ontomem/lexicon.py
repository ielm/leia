    ...
    def senses(self) -> List["Sense"]:
        return list(
            itertools.chain.from_iterable(
                map(lambda w: w.senses(include_synonyms=False), self.words())
            )
        )
...
class Sense(object):
    ...
    def _index(self, contents: dict):
        ...
        self.meaning_procedures = list(
            map(lambda mp: MeaningProcedure(mp), contents["MEANING-PROCEDURES"])
        )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "MEANING-PROCEDURES": list(
                map(lambda mp: mp.to_dict(), self.meaning_procedures)
            ),
...
class SynStruc(object):
    ...
    class DependencyElement(Element):
        ...
        def to_dict(self) -> dict:
            return {
                ...
                "deptype": self.type,
                "gov": (
                    self.governor if self.governor is None else self.governor.to_dict()
                ),
                "dep": (
                    self.dependent
                    if self.dependent is None
                    else self.dependent.to_dict()
                ),
                "var": self.variable,
                "opt": self.optional,
            }
    ...
    class ConstituencyElement(Element):
        ...
        def parse(cls, data: dict) -> "SynStruc.ConstituencyElement":
            ...
            return SynStruc.ConstituencyElement(
                ...
                list(
                    map(
                        lambda child: element_map[child["type"]].parse(child),
                        data["children"],
                    )
                ),
                data["var"] if "var" in data else None,
        ...
        def element_for_variable(
            self, variable: int
        ) -> Union["SynStruc.Element", None]:
    ...
    def _index(self, contents: List[dict]):
        ...
        self.elements = list(
            map(lambda element: element_map[element["type"]].parse(element), contents)
        )
...
class SemStruc(object):
    ...
    def elements(self, bound_variable_filter: Set[str] = None) -> List[
        Union[
            "SemStruc.Head",
            "SemStruc.Sub",
            "SemStruc.RefSem",
    ...
    def variables(
        self, bound_variable_filter: Set[str] = None
    ) -> List["SemStruc.Variable"]:
        return list(
            filter(
                lambda e: isinstance(e, SemStruc.Variable),
                self.elements(bound_variable_filter=bound_variable_filter),
            )
        )
    ...
    def properties(self, bound_variable_filter: Set[str]) -> List["SemStruc.Property"]:
        return list(
            filter(
                lambda e: isinstance(e, SemStruc.Property),
                self.elements(bound_variable_filter=bound_variable_filter),
            )
        )
