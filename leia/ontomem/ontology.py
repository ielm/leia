...
class Concept(object):
    ...
    def __init__(
        self, memory: Memory, name: str, contents: dict = None, root: "Concept" = None
    ):
        self.memory = memory
        self.name = name
        self.local = dict()
    ...
    def _index(self, contents: dict):
        ...
        if self._root == self:
            ...
            for concept in contents["private"].keys():
                if concept[0] == "@":
                    self.private[concept[1:]] = Concept(
                        self.memory, concept[1:], root=self
                    )
                elif concept[0] == "&":
                    self.private[concept[1:]] = OSet(
                        self.memory, concept[1:], root=self
                    )
    ...
    def add_local(
        self, property: str, facet: str, filler: FILLER, measured_in: str = None
    ):
    ...
    def remove_local(self, property: str, facet: str, filler: FILLER):
        ...
        self.local[property][facet] = list(
            filter(lambda f: f["value"] != filler, self.local[property][facet])
        )
    ...
    def remove_block(self, property: str, facet: str, filler: FILLER):
        ...
        self.block[property][facet] = list(
            filter(lambda f: f != filler, self.block[property][facet])
        )
    ...
    def is_blocking(self, property: str, facet: str, filler: FILLER) -> bool:
        ...
        return (
            filler in self.block[property][facet] or "*" in self.block[property][facet]
        )
    ...
    def rows(self) -> List[FILLER]:
                ...
                for filler in fillers:
                    if filler["value"] == Concept.INHFLAG:
                        ...
                        if (
                            ...
                            and len(range) > 0
                            and isinstance(range[0], Concept)
                        ):
                            pass
                        ...
                        del meta["value"]
                        out.append(
                            Concept.LocalRow(self, slot, facet, filler["value"], meta)
                        )
        ...
        for parent in self.parents():
            ...
            parent_rows = filter(
                lambda r: not isinstance(r, Concept.BlockedRow), parent_rows
            )
            parent_rows = filter(
                lambda r: not self.is_blocking(r.property, r.facet, r.filler),
                parent_rows,
            )
    ...
    def evaluate(self, slot: str, filler) -> float:
        ...
        scores = list(
            map(
                lambda f: self._evaluate_filler(filler, f, 1.0),
        ...
        for facet in [
            ...
            scores.extend(
                map(
                    lambda f: self._evaluate_filler(filler, f, facet[1]),
                    self.fillers(slot, facet[0]),
                )
            )
