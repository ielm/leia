    ...
    def run(self, sentences: List[str]) -> Analysis:
        ...
        stages = [
            {
                "name": "preprocess",
                "message": "preprocess",
                "exec": self._run_stage_preprocessor,
            },
            {
                "name": "syntax",
                "message": "run spacy syntax",
                "exec": self._run_stage_syntax,
            },
            {
                "name": "build-lex",
                "message": "build wm-lexicon",
                "exec": self._run_stage_build_lexicon,
            },
            {
                "name": "transform",
                "message": "run transformations",
                "exec": self._run_stage_transformations,
            },
            {
                "name": "synmap",
                "message": "run synmapping",
                "exec": self._run_stage_synmapping,
            },
            {
                "name": "basic-sem",
                "message": "run basic semantic analysis",
                "exec": self._run_stage_basic_semantics,
            },
        ]
        analysis = None
        ...
        if self.config.cache_read_level == "syntax":
            ...
            if cached is not None:
                ...
                stages = list(
                    filter(lambda stage: stage["name"] != "preprocess", stages)
                )
...
if __name__ == "__main__":
    ...
    if len(arguments) < 2 or (
        ...
        print(
            'Optional config parameter: runner.py config=ontosem.yml "Input text here."'
        )
