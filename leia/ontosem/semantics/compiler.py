...
class SemanticCompiler(object):
    ...
    def expand_candidates(self, synmap: SynMap) -> Iterable[Candidate]:
        return map(
            lambda senses: Candidate(self.memory, *senses),
            itertools.product(*synmap.words),
        )
    ...
    def generate_frames(self, candidate: Candidate) -> Iterable[TMRInstance]:
        ...
        for sense_map in candidate.senses:
            ...
            for refsem in sense.semstruc.refsems():
                ...
                frame = candidate.basic_tmr.new_instance(head.concept)
                frame.resolutions.add(
                    candidate.bind(sense_map.word.index, refsem, frame)
                )
                yield frame
            ...
            bound_variables = set(
                map(
                    lambda i: i[0],
                    filter(lambda i: i[1] is not None, sense_map.bindings.items()),
                )
            )
            for i, prop in enumerate(sense.semstruc.properties(bound_variables)):
                ...
                if i == 0 and head is None:
                    frame.resolutions.add(
                        candidate.bind(sense_map.word.index, SemStruc.Head(), frame)
                    )
                yield frame
    ...
    def bind_variables(self, candidate: Candidate):
        ...
        for sense_map in candidate.senses:
            ...
            if frame is not None:
                frame.resolutions.add(
                    candidate.bind(sense_map.word.index, SemStruc.Variable(0), frame)
                )
            ...
            for k, v in sense_map.bindings.items():
                ...
                frame.resolutions.add(
                    candidate.bind(
                        sense_map.word.index, SemStruc.Variable(variable), frame
                    )
                )
    ...
    def populate_frames(self, candidate: Candidate):
        ...
        for sense_map in candidate.words_by_binding_count():
            ...
            for element in sense.semstruc.elements(bound_variables):
                ...
                if isinstance(element, SemStruc.Head) or isinstance(
                    element, SemStruc.RefSem
                ):
                    self.populate_syntactic_properties(
                        frame, sense_map, element, candidate
                    )

    def populate_semantic_properties(
        self,
        frame: TMRInstance,
        sense_map: SenseMap,
        element: Union[SemStruc.Head, SemStruc.Sub, SemStruc.RefSem, SemStruc.Variable],
        candidate: Candidate,
    ):
        ...
        for k, v in element.properties():
            ...
            ):  # This is either a refsem or variable, pull it out
                v = v["VALUE"]
            ...
            if k == "SEM":
                ...
                continue
            if (
                isinstance(element, SemStruc.Head)
                and element.concept == "SET"
                and k == "MEMBER-TYPE"
            ):
                ...
                if isinstance(v, dict):
                    constraints = set()
                    if "SEM" in v:
                        constraints.add(v["SEM"])
                    if "DEFAULT" in v:
                        constraints.add(v["DEFAULT"])
            ...
            for v in v_list:
                ...
                if isinstance(v, str) and v.startswith("REFSEM"):
                    ...
                    if len(parts) == 1:
                        v = candidate.resolve(
                            sense_map.word.index,
                            SemStruc.RefSem(int(v.replace("REFSEM", ""))),
                        )
                        v = v.id()
                    else:
                        ...
                        v = candidate.resolve(
                            sense_map.word.index,
                            SemStruc.RefSem(int(parts[0].replace("REFSEM", ""))),
                        )
                        v = "%s?%s" % (v.id(), parts[1])
                    ...
                    v = candidate.resolve(
                        sense_map.word.index,
                        SemStruc.Variable(int(v.replace("^$VAR", ""))),
                    )
                    if v is None:
                        continue
    ...
    def resolve_embedded_semstruc(
        self, semstruc: list, sense_map: SenseMap, candidate: Candidate
    ) -> list:
        ...
        for e in semstruc:
            if isinstance(e, str) and e.startswith("^$VAR"):
                ...
                resolved = candidate.resolve(
                    sense_map.word.index,
                    SemStruc.Variable(int(parts[0].replace("^$VAR", ""))),
                )
    ...
    def populate_syntactic_properties(
        self,
        frame: TMRInstance,
        sense_map: SenseMap,
        element: Union[SemStruc.Head, SemStruc.RefSem],
        candidate: Candidate,
    ):
        word = sense_map.word
        ...
        if (
            frame.concept.isa(self.ontology.concept("EVENT"))
            and "V" in word.pos
            and "PAST" in word.pos
        ):
            frame.add_filler("TIME", ["<", "FIND-ANCHOR-TIME"])
        if (
            frame.concept.isa(self.ontology.concept("EVENT"))
            and "V" in word.pos
            and "PRESENT" in word.pos
        ):
            frame.add_filler("TIME", ["FIND-ANCHOR-TIME"])
        if (
            frame.concept.isa(self.ontology.concept("EVENT"))
            and "V" in word.pos
            and "INFINITIVE" in word.pos
        ):
            frame.add_filler("TIME", [">", "FIND-ANCHOR-TIME"])
    ...
    def redirect_null_sem_relations(self, candidate: Candidate):
        ...
        null_sem_frames = filter(
            lambda f: len(f.fillers("NULL-SEM")) > 0, candidate.basic_tmr.instances()
        )
            ...
            for k, v in list(frame.properties.items()):
                for filler in map(lambda x: x.value(), v):
                    if (
                        ...
                        or filler not in null_sem_replacements.keys()
                    ):
                        continue
    ...
    def build_mp_frames(self, candidate: Candidate):
        ...
        for sense_map in candidate.senses:
            ...
            for meaning_procedure in sense.meaning_procedures:
                ...
                parameters = self.resolve_embedded_mp_parameters(
                    candidate, sense_map.word, meaning_procedure.parameters()
                )
                for parameter in parameters:
                    mp_frame.add_filler("PARAMETERS", parameter)

    def resolve_embedded_mp_parameters(
        self, candidate: Candidate, word: Union[int, Word], parameters: list
    ) -> Iterable:
        ...
        def resolve_var(var: str) -> Union[str, None]:
            ...
            frame = candidate.resolve(
                word, SemStruc.Variable(int(parts[0].replace("^$VAR", "")))
            )
        ...
        def resolve_ref(ref: str) -> Union[str, None]:
            parts = ref.split(".")
            frame = candidate.resolve(
                word, SemStruc.RefSem(int(parts[0].replace("REFSEM", "")))
            )
        ...
        for parameter in parameters:
            if isinstance(parameter, list):
                if len(parameter) == 2 and parameter[0] == "VALUE":
                    ...
                    yield from self.resolve_embedded_mp_parameters(
                        candidate, word, parameter
                    )
