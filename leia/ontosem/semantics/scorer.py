...
class SemanticScorer(object):
    ...
    def score_sense_maps(self, candidate: Candidate) -> List[SenseMapScore]:
        ...
        return list(
            map(lambda sm: SenseMapScore(_score_sense_map(sm), sm), candidate.senses)
        )
            ...
            for property, fillers in frame.properties.items():
                ...
                descendants = list(
                    itertools.chain.from_iterable(
                        map(lambda r: r.descendants(), ranges)
                    )
                )
                ...
                for filler in map(lambda f: f.value(), fillers):
                    ...
                    if (
                        filler.concept.name in ranges
                        or filler.concept.name in descendants
                    ):
                    ...
                    common_ancestors = itertools.chain.from_iterable(
                        map(
                            lambda r: self.ontology.common_ancestors(
                                filler.concept.name, r
                            ),
                            ranges,
                        )
                    )
                    ...
                    distance = min(
                        map(
                            lambda a: self.ontology.distance_to_ancestor(
                                filler.concept.name, a
                            ),
                            common_ancestors,
                        )
                    )
    ...
    def score_lexical_constraints(
        self, candidate: Candidate
    ) -> List[LexicalConstraintScore]:
        results = []
        ...
        for constraint in candidate.constraints:
            ...
            if constraint.frame.concept.name == "SET":
                if "MEMBER-TYPE" in constraint.frame.properties:
                    tmr_frame_concept_options = set(
                        map(
                            lambda m: self.ontology.concept(m),
                            constraint.frame.fillers("MEMBER-TYPE"),
                        )
                    )
                elif "ELEMENTS" in constraint.frame.properties:
                    tmr_frame_concept_options = set(
                        map(
                            lambda e: candidate.basic_tmr.instance(e).concept,
                            constraint.frame.fillers("ELEMENTS"),
                        )
                    )
            ...
            for tmr_frame_concept in tmr_frame_concept_options:
                ...
                for expected_concept in constraint.concepts:
                    ...
                    if self.properties.is_property(expected_concept):
                        if (
                            constraint.frame.concept.isa(
                                self.ontology.concept("ABSTRACT-OBJECT")
                            )
                            and constraint.frame.value("REPRESENTS") == expected_concept
                        ):
                            scores.append(1.0)
                        else:
                            scores.append(0.0)
                    ...
                    # Otherwise, gain a 1.0 if it is, a 0.1 if it is not, or a score between depending on distance to a common ancestor
                        ...
                        if tmr_frame_concept.isa(expected_concept):
                            ...
                            distance = self.ontology.distance_to_ancestor(
                                expected_concept.name, tmr_frame_concept.name
                            )
