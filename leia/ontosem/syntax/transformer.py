...
class LexicalTransformer(object):
    ...
    def root_for_match(self, match: SynMatcher.SynMatchResult) -> Union[Word, None]:
        ...
        self.analysis.log("Unknown component from SynMatcher.SynMatchResult: $component", type="error", level="WARN", source=self.__class__, component=root.__class__.__name__)
        return None

    def align_syn_strucs(self, synstruc1: SynStruc, synstruc2: SynStruc) -> List[Tuple[SynStruc.Element, SynStruc.Element]]:
    ...
    def _do_elements_match(self, element1: SynStruc.Element, element2: SynStruc.Element) -> bool:
        if isinstance(element1, SynStruc.RootElement) and isinstance(element2, SynStruc.RootElement):
            return self._do_root_elements_match(element1, element2)
        if isinstance(element1, SynStruc.TokenElement) and isinstance(element2, SynStruc.TokenElement):
            return self._do_token_elements_match(element1, element2)
        if isinstance(element1, SynStruc.DependencyElement) and isinstance(element2, SynStruc.DependencyElement):
            return self._do_dependency_elements_match(element1, element2)
        if isinstance(element1, SynStruc.ConstituencyElement) and isinstance(element2, SynStruc.ConstituencyElement):
            return self._do_constituency_elements_match(element1, element2)

        # The types are not the same, or are invalid
        return False

    def _do_root_elements_match(self, element1: SynStruc.RootElement, element2: SynStruc.RootElement) -> bool:
        return True

    def _do_token_elements_match(self, element1: SynStruc.TokenElement, element2: SynStruc.TokenElement) -> bool:
