...
from leia.ontoagent.engine.module import (
    OntoAgentModule,
...
class OntoAgentModuleTestCase(TestCase):
    ...
    def test_remote_signal_calls_request(self, mock_url_open: MagicMock):
        ...
        expected_request = Request(
            "http://localhost:1234/%s/signal" % m.name,
...
class OntoAgentModuleBlueprintTestCase(TestCase):
    ...
    def test_kwargs_passed_through_blueprint(self):
        bp = OntoAgentModuleBlueprint(
            OntoAgentModule(self.a, "test"), template_folder="/test/template/folder"
        )
    ...
    def test_signal(self):
        ...
        app.test_client().post(
            "/signal", data=json.dumps(data), content_type="application/json"
        )
    ...
    def test_heartbeat_start_and_stop(self):
        ...
        data = {"time": 0.01}
        app.test_client().post(
            "/heartbeat/start", data=json.dumps(data), content_type="application/json"
        )
