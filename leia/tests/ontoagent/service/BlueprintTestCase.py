...
class OntoAgentBlueprintTestCase(TestCase):
    ...
    def test_build_payload_modules(self):
        ...
        self.assertEqual(
            [
                {"id": m1.name, "service": None},
                {"id": m2.name, "service": "http://localhost:1234/%s" % m2.name},
            ],
            payload["modules"],
        )
    ...
    def test_build_payload_scripts(self):
        ...
        self.assertEqual(
            {"available": ["test1", "test2"], "current": None}, payload["scripts"]
        )
    ...
    def test_output_instance_basics(self):
        ...
        self.assertEqual(
            {"id": i.id(), "fillers": [], "instance-of": "@frame"},
    ...
    def test_output_filler_types(self):
        ...
        i.set_filler("X", 1)
        self.assertEqual(
            [{"slot": "X", "filler": 1, "type": "attribute/number"}],
            OntoAgentBlueprint(self.a, "test", __name__).output_instance(i)["fillers"],
        )

        i.set_filler("X", 1.5)
        self.assertEqual(
            [{"slot": "X", "filler": 1.5, "type": "attribute/number"}],
            OntoAgentBlueprint(self.a, "test", __name__).output_instance(i)["fillers"],
        )

        i.set_filler("X", True)
        self.assertEqual(
            [{"slot": "X", "filler": True, "type": "attribute/boolean"}],
            OntoAgentBlueprint(self.a, "test", __name__).output_instance(i)["fillers"],
        )

        i.set_filler("X", "a")
        self.assertEqual(
            [{"slot": "X", "filler": "a", "type": "attribute/text"}],
            OntoAgentBlueprint(self.a, "test", __name__).output_instance(i)["fillers"],
        )

        i.set_filler("X", i)
        self.assertEqual(
            [{"slot": "X", "filler": repr(i), "type": "relation/instance"}],
            OntoAgentBlueprint(self.a, "test", __name__).output_instance(i)["fillers"],
        )
