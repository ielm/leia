from leia.ontomem.lexicon import (
    Lexicon,
    MeaningProcedure,
    SemStruc,
    Sense,
...
class LexiconTestCase(TestCase):
    ...
    def test_word_uses_cache_first(self):
        test_word = Word(
            self.m, "test-word", contents={"name": "test-word", "senses": {}}
        )
    ...
    def test_create_word(self):
        ...
        self.assertEqual(
            Word(self.m, "test-word", contents={"name": "test-word", "senses": {}}),
    ...
    def test_sense(self):
        ...
        lex.cache["MAN"] = Word(
            self.m,
            "MAN",
            contents={"name": "MAN", "senses": {"MAN-N1": sense_contents}},
        )

        self.assertEqual(
            Sense(self.m, "MAN-N1", contents=sense_contents), lex.sense("MAN-N1")
        )
...
class WordTestCase(LEIATestCase):
    ...
    def test_senses_with_synonyms(self):
        ...
        self.m.lexicon.word("other").add_sense(
            Sense(self.m, "notincluded", contents=self.mockSense("notincluded"))
        )
        self.assertEqual([local_sense], word.senses())
        ...
        syn_sense = Sense(
            self.m, "synsense", contents=self.mockSense("synsense", synonyms=["test"])
        )
...
class SenseTestCase(TestCase):
    ...
    def test_index_sem_struc(self):
        sense = Sense(self.m, "MAN-N1", contents=self.sample_sense_dict())
        self.assertEqual(SemStruc({"HUMAN": {"GENDER": "MALE"}}), sense.semstruc)

    def test_index_meaning_procedures(self):
        sense = Sense(self.m, "MAN-N1", contents=self.sample_sense_dict())
        self.assertEqual(
            [MeaningProcedure(["TEST-MP", "$VAR0", "$VAR1"])], sense.meaning_procedures
        )
...
class SynStrucTestCase(TestCase):
    ...
    def test_index_root(self):
        ...
        synstruc = SynStruc(contents=[{"type": "root"}])
    ...
    def test_index_token(self):
        ...
        self.assertEqual(
            [SynStruc.TokenElement({"A", "B"}, None, {"a": 1, "b": 2}, None, False)],
            synstruc.elements,
        )

        # No lemmas is represented with an empty list.
        synstruc = SynStruc(
            contents=[{"type": "token", "lemma": [], "pos": "N", "morph": {}}]
        )

        self.assertEqual(
            [SynStruc.TokenElement(set(), "N", {}, None, False)], synstruc.elements
        )

        # Both lemmas and POS can be present.
        synstruc = SynStruc(
            contents=[{"type": "token", "lemma": ["A", "B"], "pos": "N", "morph": {}}]
        )

        self.assertEqual(
            [SynStruc.TokenElement({"A", "B"}, "N", {}, None, False)], synstruc.elements
        )
        ...
        self.assertEqual(
            [SynStruc.TokenElement({"A"}, None, {}, 3, True)], synstruc.elements
        )

    def test_index_dependency(self):
        # Dependencies must have a type.
        synstruc = SynStruc(contents=[{"type": "dependency", "deptype": "subject"}])

        self.assertEqual(
            [SynStruc.DependencyElement("subject", None, None, None, False)],
            synstruc.elements,
        )
            ...
            contents=[
                {
                    "type": "dependency",
                    "deptype": "subject",
                    "gov": {"type": "token", "lemma": [], "pos": "N", "morph": {}},
                    "dep": {"type": "token", "lemma": [], "pos": "V", "morph": {}},
                }
            ]
        )
        ...
        self.assertEqual(
            [
                SynStruc.DependencyElement(
                    "subject",
                    SynStruc.TokenElement(set(), "N", dict(), None, False),
                    SynStruc.TokenElement(set(), "V", dict(), None, False),
                    None,
                    False,
                )
            ],
        ...
        self.assertEqual(
            [SynStruc.DependencyElement("subject", None, None, 3, True)],
            synstruc.elements,
        )
    ...
    def test_index_constituency(self):
        ...
        synstruc = SynStruc(
            contents=[{"type": "constituency", "contype": "VP", "children": []}]
        )

        self.assertEqual(
            [SynStruc.ConstituencyElement("VP", [], None, False)], synstruc.elements
        )
        ...
        self.assertEqual(
            [SynStruc.ConstituencyElement("VP", [], 3, True)], synstruc.elements
        )
    ...
    def test_element_for_variable(self):
        ...
        self.assertEqual(
            synstruc.elements[1].children[0], synstruc.element_for_variable(2)
        )
        self.assertEqual(
            synstruc.elements[1].children[0].children[0],
            synstruc.element_for_variable(4),
        )
        self.assertEqual(
            synstruc.elements[1].children[1], synstruc.element_for_variable(6)
        )
