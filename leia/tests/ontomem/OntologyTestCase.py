...
class OntologyTestCase(TestCase):
    ...
    def test_common_ancestors(self):
        ...
        self.assertEqual(
            {"GRANDPARENT", "PARENT1"}, ontology.common_ancestors("CHILD1", "CHILD2")
        )
        self.assertEqual({"GRANDPARENT"}, ontology.common_ancestors("CHILD1", "CHILD3"))
        self.assertEqual(
            {"GRANDPARENT"}, ontology.common_ancestors("PARENT1", "PARENT2")
        )
...
class ConceptTestCase(TestCase):
    ...
    def test_parse_local_properties(self):
        ...
        self.assertEqual(
            [{"value": "ABC"}, {"value": "DEF"}], concept.local["a"]["sem"]
        )
        ...
        self.assertEqual([{"value": WILDCARD.ANYLIT}], concept.local["a"]["sem"])

        self.assertEqual([{"value": Concept.INHFLAG}], concept.local["b"]["sem"])
        ...
        self.assertEqual([{"value": (COMPARATOR.GT, 1)}], concept.local["a"]["sem"])

        self.assertEqual(
            [{"value": (COMPARATOR.BETWEEN, 0.0, 1.0)}], concept.local["b"]["sem"]
        )
    ...
    def test_parse_block_properties(self):
        ...
        self.assertEqual(["ABC", "DEF"], concept.block["a"]["sem"])
    ...
    def test_parse_sets(self):
        ...
        self.assertEqual(
            [private_p1, p2, s2, prop1], s1.members()
        )  # Note that @p1 uses the private concept
    ...
    def test_ancestors(self):
        ...
        self.assertEqual(
            {parent1, parent2, grandparent1, grandparent2, grandparent3},
            concept.ancestors(),
        )
    ...
    def test_descendants(self):
        ...
        self.assertEqual(
            {parent1, parent2, child1, child2, child3}, grandparent.descendants()
        )
    ...
    def test_rows_includes_local(self):
        ...
        self.assertEqual(
            [
                Concept.LocalRow(
                    concept, "color", "sem", ["red", "green", "blue"], dict()
                )
            ],
            concept.rows(),
        )
        ...
        self.assertEqual(
            [
                ...
                Concept.LocalRow(
                    concept, "height", "sem", [">", 0], {"measured-in": "inches"}
                ),
            ],
            concept.rows(),
        )
    ...
    def test_rows_includes_inherited(self):
        ...
        self.assertEqual(
            [
                ...
                Concept.LocalRow(
                    parent, "height", "sem", [">", 0], {"measured-in": "inches"}
                ),
                Concept.LocalRow(
                    grandparent, "width", "sem", [">", 0], {"measured-in": "inches"}
                ),
            ],
            concept.rows(),
        )
    ...
    def test_fillers(self):
        ...
        self.assertEqual({"a", "c"}, set(concept.fillers("prop", "sem")))
    ...
    def test_private_frames(self):
        ...
        public = self.m.ontology.concept(
            "PRIVATE"
        )  # Same literal name as the private frame
...
class ConceptEvaluateTestCase(TestCase):
    ...
    def test_simple_literal(self):
        ...
        self.assertEqual(
            0.9, concept.evaluate("literal1", "A")
        )  # The value is part of the local definition
        self.assertEqual(
            0.0, concept.evaluate("literal1", "D")
        )  # The value is not part of the local definition

        self.assertEqual(
            0.9, concept.evaluate("literal2", "A")
        )  # literal2 is a wildcard for any str
        self.assertEqual(
            0.9, concept.evaluate("literal2", "D")
        )  # literal2 is a wildcard for any str
    ...
    @skip(
        "Skip this for now; requires SETs to be defined in the episodic memory to properly evaluate."
    )
