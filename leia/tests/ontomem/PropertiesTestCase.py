...
class PropertyInventoryTestCase(TestCase):
    ...
    def test_properties_with_type(self):
        ...
        self.assertEqual(
            ...
            set(
                map(
                    lambda p: p.name,
                    memory.properties.properties_with_type(Property.TYPE.RELATION),
                )
            ),
        )
        self.assertEqual(
            ...
            set(
                map(
                    lambda p: p.name,
                    memory.properties.properties_with_type(Property.TYPE.LITERAL),
                )
            ),
        )
        self.assertEqual(
            ...
            set(
                map(
                    lambda p: p.name,
                    memory.properties.properties_with_type(Property.TYPE.BOOLEAN),
                )
            ),
        )
    ...
    def test_inverses(self):
        memory = Memory()
        memory.properties.add_property(
            Property(memory, "RELATION", {"type": "relation"})
        )
        memory.properties.add_property(
            Property(memory, "R1", {"type": "relation", "inverse": "@R1-OF"})
        )
        memory.properties.add_property(
            Property(memory, "R2", {"type": "relation", "inverse": "@R2-OF"})
        )
