...
class SemanticCompilerTestCase(LEIATestCase):
    ...
    def test_expand_candidates(self):
        ...
        candidates = list(SemanticCompiler(self.analysis).expand_candidates(synmap))
    ...
    def test_generate_frames(self):
        ...
        lexicon.add_sense(sm3.word, Sense(self.m, "TEST-T3", contents=s3))
        ...
        self.assertEqual(
            "COLOR.1", candidate.resolve(1, SemStruc.Property(2, "COLOR")).id()
        )
        self.assertEqual(
            "AGENT.1", candidate.resolve(1, SemStruc.Property(2, "AGENT")).id()
        )
        self.assertEqual(
            "THING.1", candidate.resolve(2, SemStruc.Property(4, "THING")).id()
        )
        self.assertEqual("THING.1", candidate.resolve(2, SemStruc.Head()).id())
        self.assertEqual(
            "AGE.1", candidate.resolve(2, SemStruc.Property(4, "AGE")).id()
        )
    ...
    def test_bind_variables(self):
        ...
        sm3 = SenseMap(
            Word.basic(7), "TEST-T3", {"$VAR9": 9}
        )  # Word 9 does not exist, this will be skipped.
    ...
    def test_populate_frames(self):
        ...
        s1 = self.mockSense(
            "TEST-T1",
            semstruc={"HEAD": {"head1": "content"}, "SUB": {"sub1": "content"}},
        )

        s2 = self.mockSense(
            "TEST-T2",
            semstruc={"HEAD": {"head2": "content"}, "^$VAR1": {"ABC": "content"}},
        )
    ...
    def test_populate_semantic_properties(self):
        ...
        element = SemStruc.Head(
            "TEST",
            {"AGENT": "HUMAN", "COLOR": "RED", "TIME": [">", "FIND-ANCHOR-TIME"]},
        )
        ...
        analyzer = SemanticCompiler(self.analysis)
        analyzer.resolve_embedded_semstruc = MagicMock(
            side_effect=analyzer.resolve_embedded_semstruc
        )
        ...
        analyzer.resolve_embedded_semstruc.assert_called_once_with(
            [">", "FIND-ANCHOR-TIME"], sense_map, candidate
        )
    ...
    def test_populate_semantic_properties_with_refsems(self):
        ...
        candidate.bind(
            0, SemStruc.RefSem(1), f1
        )  # This is REFSEM1 in word 1 (the word that is currently being populated)
        candidate.bind(
            1, SemStruc.RefSem(1), f2
        )  # This is REFSEM1 in word 2 (this should be ignored; it is not the correct binding)
    ...
    def test_populate_semantic_properties_as_variable(self):
        ...
        element = SemStruc.Variable(
            1, {"AGENT": "HUMAN", "COLOR": "RED", "TIME": [">", "FIND-ANCHOR-TIME"]}
        )

        candidate.bind(
            0, SemStruc.Variable(1), frame
        )  # This is the binding for VAR1 in word 0, pointing to the head frame for word 4
    ...
    def test_populate_semantic_properties_with_variables(self):
        ...
        candidate.bind(
            0, SemStruc.Variable(1), f1
        )  # This is the binding for VAR1 in word 0, pointing to the head frame for word 4
        candidate.bind(
            0, SemStruc.Variable(2), f2
        )  # This is the binding for VAR2 in word 0, pointing to the head frame for word 5
        candidate.bind(
            0, SemStruc.Variable(3), f3
        )  # This is the binding for VAR3 in word 0, pointing to the head frame for word 6 (this should be ignored)
    ...
    def test_populate_semantic_properties_as_variable_with_lexical_constraint(self):
        ...
        self.assertEqual(
            [Constraint(1, frame, "VOTE", sense_map)], candidate.constraints
        )  # Instead, it is added to a special "constraints" field
    ...
    def test_populate_semantic_properties_as_set_with_lexical_constraint(self):
        ...
        element = SemStruc.Head(
            "SET", {"MEMBER-TYPE": {"SEM": "HUMAN", "DEFAULT": "ANIMATE"}}
        )
        ...
        self.assertEqual(
            [], frame.fillers("MEMBER-TYPE")
        )  # This is not added as a property
        self.assertEqual(
            [Constraint(1, frame, {"HUMAN", "ANIMATE"}, sense_map)],
            candidate.constraints,
        )  # Instead, constraints are added
    ...
    def test_populate_semantic_properties_as_null_sem(self):
        ...
        element = SemStruc.Head("^$VAR2", {"NULL-SEM": "+"})
        ...
        sense = Sense(
            self.m,
            "TEST-T1",
            contents=self.mockSense("TEST-T1", semstruc={"REFSEM1": {}}),
        )
    ...
    def test_resolve_embedded_semstruc(self):
        ...
        self.assertEqual(
            semstruc, analyzer.resolve_embedded_semstruc(semstruc, sense_map, candidate)
        )
    ...
    def test_resolve_embedded_semstruc_with_variables(self):
        ...
        self.assertEqual(
            [">", ["VALUE", f1.id()], ["XYZ", ["VALUE", f2.id()]], ["NOSUCH", "$VAR4"]],
            analyzer.resolve_embedded_semstruc(semstruc, sense_map, candidate),
        )
    ...
    def test_fix_inverses(self):
        self.m.properties.add_property(
            Property(self.m, "RELATION", contents={"type": "relation"})
        )

        # No inverse is defined, so the default R-INVERSE will be used
        self.m.properties.add_property(
            Property(self.m, "R", contents={"type": "relation"})
        )
        ...
        self.assertEqual(
            [], f2.fillers("R")
        )  # Sanity check, there are no fillers for R in f2
    ...
    def test_build_mp_frames(self):
        ...
        s2 = self.mockSense(
            "TEST-T2", meaning_procedures=[["TESTMP3", "^$VAR1", ["VALUE", "^$VAR2"]]]
        )

        # Meaning procedure variables are optional; if they can't be resolved, they should resolve to None
        s3 = self.mockSense(
            "TEST-T3", meaning_procedures=[["TESTMP4", "^$VAR1", ["VALUE", "^$VAR2"]]]
        )
        ...
        mp1frame = list(
            filter(
                lambda f: f.fillers("NAME") == ["TESTMP1"],
                candidate.basic_tmr.instances(),
            )
        )[0]
        mp2frame = list(
            filter(
                lambda f: f.fillers("NAME") == ["TESTMP2"],
                candidate.basic_tmr.instances(),
            )
        )[0]
        mp3frame = list(
            filter(
                lambda f: f.fillers("NAME") == ["TESTMP3"],
                candidate.basic_tmr.instances(),
            )
        )[0]
        mp4frame = list(
            filter(
                lambda f: f.fillers("NAME") == ["TESTMP4"],
                candidate.basic_tmr.instances(),
            )
        )[0]
