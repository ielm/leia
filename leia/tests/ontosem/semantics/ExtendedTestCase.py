...
class BasicSemanticsMPProcessorTestCase(TestCase):
    ...
    @patch("leia.ontosem.semantics.extended.subprocess")
    def test_analyze(self, mock_subprocess: MagicMock):
        ...
        lisp_exe = '(post-basic-semantic-MPs "%s")' % json.dumps(
            analysis.to_dict()
        ).replace('"', '\\"')

        mock_subprocess.Popen.assert_called_once_with(
            "clisp -q --silent -M %s" % mem_file,
            shell=True,
        ...
        analyzer._assign_extended_tmrs_to_candidates.assert_called_once_with(
            analysis, {"abc": tmr}
        )
    ...
    def test_parse_lisp_to_frame(self):
        ...
        lisp = [
            ...
            ["RESOLUTIONS", ["1.HEAD", "0.VAR.1", "1.VAR.0", "2.VAR.1"]],
        ]
        ...
        self.assertEqual(
            {"1.HEAD", "0.VAR.1", "1.VAR.0", "2.VAR.1"}, instance.resolutions
        )
