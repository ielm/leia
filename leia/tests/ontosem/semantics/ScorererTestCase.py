...
from leia.ontosem.semantics.candidate import (
    LexicalConstraintScore,
    RelationRangeScore,
    SenseMapScore,
)
...
class SemanticScorerTestCase(LEIATestCase):
    ...
    def test_score_sense_maps(self):
        ...
        sense.synstruc.elements = [
            SynStruc.TokenElement({"TEST"}, None, dict(), 1, True)
        ]
        ...
        sense.synstruc.elements = [
            SynStruc.TokenElement({"TEST"}, None, dict(), 1, False)
        ]
    ...
    def test_score_relation_ranges(self):
        ...
        self.m.properties.add_property(
            Property(self.m, "REL1", contents={"type": "relation"})
        )
        self.m.properties.add_property(
            Property(self.m, "REL2", contents={"type": "relation"})
        )
        self.m.properties.add_property(
            Property(self.m, "ATTRIBUTE", contents={"type": "literal"})
        )  # TODO: THIS IS WRONG
        self.m.properties.add_property(
            Property(self.m, "ATTR1", contents={"type": "literal"})
        )
        ...
        self.assertEqual([RelationRangeScore(1.0, f1, "REL1", f2)], scores)
        ...
        scores = scorer.score_relation_ranges(candidate)
        self.assertEqual([RelationRangeScore(0.1, f1, "REL2", f2)], scores)
        ...
        f1.add_filler("REL1", f2)
        scores = scorer.score_relation_ranges(candidate)
        self.assertEqual([RelationRangeScore(0.9, f1, "REL1", f2)], scores)
    ...
    def test_score_lexical_constraints(self):
        ...
        self.assertEqual(
            [
                ...
                LexicalConstraintScore(
                    0.7, c5
                ),  # The best option of CHILD and PARENT2 is used
                LexicalConstraintScore(
                    0.1, c6
                ),  # Violation of a NOT constraint earns a maximum penalty
                LexicalConstraintScore(
                    1.0, c7
                ),  # Maximum score applied if a NOT constraint is passed
            ],
            scores,
        )
    ...
    def test_score_lexical_constraints_on_sets(self):
        ...
        frame.properties = {"MEMBER-TYPE": ["GRANDPARENT"]}
        ...
        frame.properties = {"MEMBER-TYPE": ["PARENT1", "PARENT2"]}
        ...
        frame.properties = {"ELEMENTS": [pf1.id()]}
