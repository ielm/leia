...
class SyntaxTestCase(TestCase):
    ...
    def test_from_spacy(
        self, mock_parse_lisp_results: MagicMock, mock_word_from_spacy: MagicMock
    ):
        ...
        self.assertEqual(
            [Dependency(word2, word1, "test"), Dependency(word2, word2, "ROOT")],
            syntax.dependencies,
        )

        mock_parse_lisp_results.assert_called_once_with(
            ["TEST", ["PARSE", ["STRING"]]], [word1, word2]
        )
        mock_word_from_spacy.assert_has_calls(
            [call(word1, None, None), call(word2, None, None)]
        )
...
class ConstituencyNodeTestCase(TestCase):
    ...
    def test_parse_lisp_results(self):
        ...
        parsed = ConstituencyNode.parse_lisp_results(
            ["S", ["V", "Word"], ["N", "bird"]], words
        )
...
class LispParserTestCase(TestCase):
    ...
    def test_lisp_to_list(self):
        ...
        self.assertEqual([[["XYZ", "ABC"], [["DEF", "1", "2", "3"], "BLAH"]]], results)
    ...
    def test_list_key_to_value(self):
        ...
        self.assertEqual(
            ["DEF", ["1", "2", "3"], "BLAH"], LispParser.list_key_to_value(lisp, "DEF")
        )
