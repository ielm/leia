...
class MongoConceptExporter(object):
    ...
    def __init__(
        self, url: str, port: int, database: str, collection: str, output_dir: str
    ):
        self.url = url
        self.port = port
    ...
    def run(self):
        ...
        properties = set(
            map(
                lambda e: e["name"],
                filter(
                    lambda e: e["name"] == "property" or "property" in e["ancestry"],
                    entries,
                ),
            )
        )
        literals = set(
            map(
                lambda e: e["name"],
                filter(lambda e: "literal-attribute" in e["ancestry"], entries),
            )
        )
        concepts = list(
            filter(
                lambda e: e["name"] != "property" and "property" not in e["ancestry"],
                entries,
            )
        )
        ...
        for concept in concepts:
            ...
            for prop in concept["localProperties"]:
                ...
                try:
                    filler = self.parse_filler(
                        slot, filler, concept_names, literals, properties
                    )
                    if filler is None:
                        continue
                except:
                ...
                if slot in default_measures:
                    prop["meta"] = {"measured-in": default_measures[slot]}
            ...
            if len(
                list(
                    filter(
                        lambda p: p["slot"] == "is-onto-instance",
                        concept["localProperties"],
                    )
                )
            ):
        ...
        print(
            "Total onto-instances with unknown owners: %d"
            % len(unmapped_onto_instances)
        )
    ...
    def parse_scalar_range(self, value) -> Union[
...
class MongoPropertyExporter(object):
    ...
    def run(self):
        ...
        aggregation = [
            ...
            {"$match": {"properties.name": "property"}},
            {
                "$project": {
                    "name": 1,
                    "definition": 1,
        ...
        for property in h.aggregate(aggregation):
            ...
            ancestry = set(
                map(lambda parent: parent["name"].upper(), property["properties"])
            )
            ...
            if "is-onto-instance" in set(
                map(lambda p: p["slot"], property["localProperties"])
            ):
                skipped.append(name)
                continue
...
class MongoWordExporter(object):
    ...
    def sanitize_sense(self, sense: dict) -> dict:
        ...
        sense["EXAMPLE-BINDINGS"] = list(
            map(lambda eb: str(eb), sense["EXAMPLE-BINDINGS"])
        )
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "properties":
        output_dir = "%s/leia/knowledge/properties" % os.getcwd()
        exporter = MongoPropertyExporter(
            "localhost", 27016, "leia-ontology", "canonical-v.1.0.6", output_dir
        )
        exporter.run()
    elif sys.argv[1] == "concepts":
        output_dir = "%s/leia/knowledge/concepts" % os.getcwd()
        exporter = MongoConceptExporter(
            "localhost", 27016, "leia-ontology", "canonical-v.1.0.6", output_dir
        )
        exporter.run()
    elif sys.argv[1] == "words":
        output_dir = "%s/leia/knowledge/words" % os.getcwd()
        exporter = MongoWordExporter(
            "localhost", 27016, "leia-lexicon", "canonical-v.1.3.0", output_dir
        )
        exporter.run()
