...
class FormatToLISP(object):
    ...
    def sense_to_lisp(self, sense: dict) -> str:
        ...
        tmrhead = self.tmr_head_to_lisp(
            sense["TMR-HEAD"] if "TMR-HEAD" in sense else "NIL"
        )
        synstruc = self.synstruc_to_lisp(sense["SYN-STRUC"])
        semstruc = self.semstruc_to_lisp(sense["SEM-STRUC"])
        output_syntax = self.output_syntax_to_lisp(
            sense["OUTPUT-SYNTAX"] if "OUTPUT-SYNTAX" in sense else "NIL"
        )
        meaning_procedures = self.meaning_procedures_to_lisp(
            sense["MEANING-PROCEDURES"] if "MEANING-PROCEDURES" in sense else "NIL"
        )

        example_bindings = self.example_bindings_to_lisp(
            sense["EXAMPLE-BINDINGS"] if "EXAMPLE-BINDINGS" in sense else "NIL"
        )
        example_deps = self.example_deps_to_lisp(
            sense["EXAMPLE-DEPS"] if "EXAMPLE-DEPS" in sense else "NIL"
        )
        ...
        return template.format(
            ...
            sem=semstruc,
            out=output_syntax,
            mp=meaning_procedures,
            exbind=example_bindings,
            exdep=example_deps,
            synonyms=synonyms,
            hyponyms=hyponyms,
            types=types,
            usewithtypes=use_with_types,
        )
    ...
    def _flatten_synstruc(self, synstruc: Union[str, int, float, list, OrderedDict]):
        if isinstance(synstruc, dict):
            return "(%s)" % " ".join(
                map(
                    lambda i: "(%s %s)"
                    % (self._tidy_synstruc_header(i[0]), self._flatten_synstruc(i[1])),
                    synstruc.items(),
                )
            )
    ...
    def _flatten_semstruc(self, semstruc: Union[str, dict, list]) -> str:
        if isinstance(semstruc, dict):
            ...
            for k, v in semstruc.items():
                if k == "CONSTRAINT":
                    elements.append(
                        "(%s)" % " ".join(map(lambda i: self._flatten_semstruc(i), v))
                    )
                elif v == dict():
                    ...
                    elements.append(
                        "(%s %s)"
                        % (self._tidy_semstruc_header(k), self._flatten_semstruc(v))
                    )
    ...
    def meaning_procedures_to_lisp(
        self, meaning_procedures: Union[str, List[Union[str, List]]]
    ) -> str:
        ...
        return "(MEANING-PROCEDURES %s)" % " ".join(
            map(lambda e: self._flatten_mp(e), meaning_procedures)
        )
    ...
    def example_bindings_to_lisp(
        self, example_bindings: Union[str, List[str], List[List[str]]]
    ):
        if example_bindings == "":
        ...
        if list not in types:
            return "(EXAMPLE-BINDINGS (%s))" % " ".join(
                map(lambda eb: str(eb), example_bindings)
            )

        return "(EXAMPLE-BINDINGS %s)" % " ".join(
            map(
                lambda eb: "(%s)" % " ".join(map(lambda x: str(x), eb)),
                example_bindings,
            )
        )
    ...
    def example_deps_to_lisp(self, example_deps: Union[str, List[List[str]]]):
        ...
        return "(EXAMPLE-DEPS (%s))" % " ".join(
            map(lambda ed: "(%s)" % " ".join(map(lambda e: str(e), ed)), example_deps)
        )
    ...
    def synonyms_to_lisp(self, synonyms: Union[str, int, List[Union[str, int]]]) -> str:
        ...
        return "(SYNONYMS (%s))" % " ".join(
            map(
                lambda s: self._escape_word_name(s) if isinstance(s, str) else str(s),
                synonyms,
            )
        )
    ...
    def hyponyms_to_lisp(self, hyponyms: Union[str, int, List[Union[str, int]]]) -> str:
        ...
        return "(HYPONYMS (%s))" % " ".join(
            map(
                lambda h: self._escape_word_name(h) if isinstance(h, str) else str(h),
                hyponyms,
            )
        )
    ...
    def _escape_sense_name(self, name: str):
        ...
        if (
            len(list(filter(lambda c: self._is_quotable_character(c), name_trimmed)))
            == 0
        ):
            return name
        return '"%s"' % name
...
class FormatFromLISP(object):
    ...
    def list_to_sense(self, list: List) -> dict:
        ...
        return {
            ...
            "COMMENTS": self.parse_comments(
                self.list_key_to_value(list, "COMMENTS", "")
            ),
            "TMR-HEAD": self.parse_tmr_head(
                self.list_key_to_value(list, "TMR-HEAD", "NIL")
            ),
            "SYN-STRUC": self.parse_syn_struc(
                self.list_key_to_value(list, "SYN-STRUC", [])
            ),
            "SEM-STRUC": self.parse_sem_struc(
                self.list_key_to_value(list, "SEM-STRUC", "ALL")
            ),
            "OUTPUT-SYNTAX": self.parse_output_syntax(
                self.list_key_to_value(list, "OUTPUT-SYNTAX", "NIL")
            ),
            "MEANING-PROCEDURES": self.parse_meaning_procedures(
                self.list_key_to_value(list, "MEANING-PROCEDURES", "NIL")
            ),
            "EXAMPLE-BINDINGS": self.parse_example_bindings(
                self.list_key_to_value(list, "EXAMPLE-BINDINGS", "NIL")
            ),
            "EXAMPLE-DEPS": self.parse_example_deps(
                self.list_key_to_value(list, "EXAMPLE-DEPS", "NIL")
            ),
            "SYNONYMS": self.parse_synonyms(
                self.list_key_to_value(list, "SYNONYMS", "NIL")
            ),
            "HYPONYMS": self.parse_hyponyms(
                self.list_key_to_value(list, "HYPONYMS", "NIL")
            ),
            "TYPES": self.parse_types(self.list_key_to_value(list, "TYPES", [])),
            "USE-WITH-TYPES": self.parse_use_with_types(
                self.list_key_to_value(list, "USE-WITH-TYPES", [])
            ),
        }

    def list_key_to_value(
        self, lisp: list, key: str, default=None
    ) -> Union[List, None]:
        for element in lisp:
    ...
    def parse_sem_struc(self, contents: List):
        ...
        if (
            isinstance(contents[1], list)
            and len(contents[1]) == 1
            and isinstance(contents[1][0], str)
            and len(contents) == 2
        ):
            return contents[1][0]
