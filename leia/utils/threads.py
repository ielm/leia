...
def multiprocess_read_directory(
    ...
    if attempt_multiprocess():
        pool = Pool(4)
        contents = pool.starmap(
            multiprocess_read_json_file,
            map(lambda file: (directory, file, extension), files),
        )
        return contents
...
def multiprocess_read_json_file(
    directory: str, file_name: str, extension: str
) -> Tuple[str, dict]:
