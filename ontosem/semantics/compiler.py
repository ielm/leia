...
class SemanticCompiler(object):
    ...
    def expand_candidates(self, synmap: SynMap) -> Iterable[Candidate]:
        return map(lambda senses: Candidate(*senses), itertools.product(*synmap.words))
    ...
    def generate_frames(self, candidate: Candidate) -> Iterable[TMRFrame]:
        ...
        for sense_map in candidate.senses:
            ...
            if head is not None:
                frame = candidate.basic_tmr.next_frame_for_concept(head.concept)
            ...
            for sub in sense.semstruc.subs():
                frame = candidate.basic_tmr.next_frame_for_concept(sub.concept)
            ...
            for i, prop in enumerate(sense.semstruc.properties(bound_variables)):
                frame = candidate.basic_tmr.next_frame_for_concept(prop.property)
    ...
    def populate_syntactic_properties(self, frame: TMRFrame, sense_map: SenseMap, element: Union[SemStruc.Head, SemStruc.RefSem], candidate: Candidate):
        ...
        if self.ontology.concept(frame.concept).isa(self.ontology.concept("EVENT")) and "V" in word.pos and "PAST" in word.pos:
            frame.add_filler("TIME", ["<", "FIND-ANCHOR-TIME"])
        if self.ontology.concept(frame.concept).isa(self.ontology.concept("EVENT")) and "V" in word.pos and "PRESENT" in word.pos:
            frame.add_filler("TIME", ["FIND-ANCHOR-TIME"])
        if self.ontology.concept(frame.concept).isa(self.ontology.concept("EVENT")) and "V" in word.pos and "INFINITIVE" in word.pos:
            frame.add_filler("TIME", [">", "FIND-ANCHOR-TIME"])
    ...
    def redirect_null_sem_relations(self, candidate: Candidate):
        ...
        null_sem_frames = filter(lambda f: len(f.fillers("NULL-SEM")) > 0, candidate.basic_tmr.frames.values())
        ...
        for frame in null_sem_frames:
            for responsible_frame in frame.fillers("NULL-SEM"):
                if responsible_frame == frame.frame_id():
                    continue
                null_sem_replacements[frame.frame_id()] = responsible_frame
                break
    ...
    def remove_null_sems(self, candidate: Candidate):
        ...
        for frame in list(candidate.basic_tmr.frames.values()):
            if len(frame.fillers("NULL-SEM")) > 0:
                candidate.basic_tmr.remove_frame(frame)
            ...
            for property in list(frame.properties.keys()):
                ...
                for filler in frame.fillers(property):
                    if filler not in candidate.basic_tmr.frames:
                        continue
                    filler_frame = candidate.basic_tmr.frames[filler]

                    filler_frame.add_filler(inverses[property], frame.frame_id())
    ...
    def build_mp_frames(self, candidate: Candidate):
        ...
        for sense_map in candidate.senses:
            ...
            for meaning_procedure in sense.meaning_procedures:
                mp_frame = candidate.basic_tmr.next_frame_for_concept("MEANING-PROCEDURE")
    ...
    def resolve_embedded_mp_parameters(self, candidate: Candidate, word: Union[int, Word], parameters: list) -> Iterable:
        ...
        def resolve_var(var: str) -> Union[str, None]:
            ...
            if frame is None:
                ...
                return "%s?%s" % (frame.frame_id(), parts[1])
