...
class BasicSemanticsMPProcessor(object):
    ...
    def _parse_lisp_tmrs(self, lisp: str) -> Dict[str, Tuple[TMR, List['ExtendedScorePlaceholder']]]:
        ...
        for candidate in data:
            ...
            if candidate_id.startswith("\""): candidate_id = candidate_id[1:]
            if candidate_id.endswith("\""): candidate_id = candidate_id[:-1]
            ...
            scores = list(map(lambda score: ExtendedScorePlaceholder.parse_lisp(score), scores))
    ...
    def _lisp_to_frame(self, tmr: TMR, input: list) -> TMRInstance:
        ...
        if resolutions == "NIL": resolutions = []
    ...
    def _assign_extended_tmrs_to_candidates(self, analysis: Analysis, extended: Dict[str, Tuple[TMR, List['ExtendedScorePlaceholder']]]):
...
class ExtendedScorePlaceholder(Score):
    ...
    def __repr__(self):
        return "ExtendedScorePlaceholder %s %f: '%s'" % (self.name, self.score, self.message)


if __name__ == "__main__":
    import pickle
    results = pickle.load(open("/Users/jesse/Desktop/analysis.p", "rb" ))
