...
from utils.formatting import FormatFromLISP, FormatToLISP
...
class FormatToLISPTestCase(TestCase):
    ...
    def mockSense(self, sense: str, definition: str = "", example: str = "",
                  comments: str = "", tmrhead: str = "NIL", synstruc: Union[OrderedDict, str] = "",
                  semstruc: Union[OrderedDict, str] = "", outputsyntax: str = "NIL",
                  meaningprocedures: Union[str, List[List[Union[str, List[str]]]]] = "NIL",
                  examplebindings: str = "NIL", exampledeps: str = "NIL", synonyms: str = "NIL",
                  hyponyms: str = "NIL", types: List[str] = None, use_with_types: List[str] = None,
                  **kwargs) -> OrderedDict:
    ...
    def test_word_to_lisp(self):
        ...
        formatter.sense_to_lisp = MagicMock(side_effect=lambda x: "%s-lisp" % x["SENSE"])
        self.assertEqual("(XYZ\nXYZ-N1-lisp\nXYZ-N2-lisp\nXYZ-N3-lisp)", formatter.word_to_lisp(word, senses))
    ...
    def test_sense_to_lisp(self):
        # Simple case
        sense = self.mockSense("TEST-N1", save=False,
    ...
    def test_synstruc_to_lisp(self):
        ...
        self.assertEqual("(SYN-STRUC ((KEY VALUE)))", FormatToLISP().synstruc_to_lisp(OrderedDict({"KEY": "VALUE"})))
        self.assertEqual("(SYN-STRUC ((KEY VALUE) (OTHER 123)))", FormatToLISP().synstruc_to_lisp(OrderedDict({"KEY": "VALUE", "OTHER": 123})))
        self.assertEqual("(SYN-STRUC ((KEY (V1 V2))))", FormatToLISP().synstruc_to_lisp(OrderedDict({"KEY": ["V1", "V2"]})))
        self.assertEqual("(SYN-STRUC ((KEY ((INNER1 ((INNER2A VALUE) (INNER2B 123)))))))", FormatToLISP().synstruc_to_lisp(OrderedDict({"KEY": {"INNER1": {"INNER2A": "VALUE", "INNER2B": 123}}})))

        # Special consideration: the database uses a -# moniker for repeating keys; these should be stripped.

        self.assertEqual("(SYN-STRUC ((KEY V1) (KEY V2)))", FormatToLISP().synstruc_to_lisp(OrderedDict({"KEY": "V1", "KEY-1": "V2"})))

        # Special consideration: the database records the extra ROOT field as ROOT-WORD; the "-WORD" should be stripped.

        self.assertEqual("(SYN-STRUC ((ROOT ABC) (ROOT DEF)))", FormatToLISP().synstruc_to_lisp(OrderedDict({"ROOT": "ABC", "ROOT-WORD": "DEF"})))
    ...
    def test_semstruc_to_lisp(self):
        ...
        self.assertEqual("(SEM-STRUC (CONCEPT))", FormatToLISP().semstruc_to_lisp("CONCEPT"))
        self.assertEqual("(SEM-STRUC (CONCEPT (ABC DEF) (XYZ 123)))", FormatToLISP().semstruc_to_lisp({"CONCEPT": {"ABC": "DEF", "XYZ": 123}}))
        self.assertEqual("(SEM-STRUC (CONCEPT (ABC (VALUE ^$VAR1))))", FormatToLISP().semstruc_to_lisp({"CONCEPT": {"ABC": {"VALUE": "^$VAR1"}}}))
        self.assertEqual("(SEM-STRUC (CONCEPT1 (ABC 1)) (CONCEPT2 (ABC 2)))", FormatToLISP().semstruc_to_lisp({"CONCEPT1": {"ABC": 1}, "CONCEPT2": {"ABC": 2}}))
        self.assertEqual("(SEM-STRUC (CONCEPT (ABC (1 2 3))))", FormatToLISP().semstruc_to_lisp({"CONCEPT": {"ABC": [1, 2, 3]}}))

        # Special consideration: the database uses a -# moniker for repeating keys; these should be stripped.

        self.assertEqual("(SEM-STRUC (CONCEPT (ABC 1) (ABC 2)))", FormatToLISP().semstruc_to_lisp({"CONCEPT": {"ABC": 1, "ABC-1": 2}}))

        # Special consideration: the database wraps top-level comparators in a CONSTRAINT field that must be removed.

        self.assertEqual("(SEM-STRUC (> (VALUE REFSEM1.X) (VALUE REFSEM2.X)))", FormatToLISP().semstruc_to_lisp({"CONSTRAINT": [">", ["VALUE", "REFSEM1.X"], ["VALUE", "REFSEM2.X"]]}))
    ...
    def test_output_syntax_to_lisp(self):
        ...
        self.assertEqual("(OUTPUT-SYNTAX NIL)", FormatToLISP().output_syntax_to_lisp(""))
        self.assertEqual("(OUTPUT-SYNTAX NIL)", FormatToLISP().output_syntax_to_lisp("NIL"))
        self.assertEqual("(OUTPUT-SYNTAX INF-CL)", FormatToLISP().output_syntax_to_lisp("INF-CL"))
        self.assertEqual("(OUTPUT-SYNTAX (INF-CL N))", FormatToLISP().output_syntax_to_lisp(["INF-CL", "N"]))
    ...
    def test_meaning_procedures_to_lisp(self):
        ...
        self.assertEqual("(MEANING-PROCEDURES NIL)", FormatToLISP().meaning_procedures_to_lisp(""))
        self.assertEqual("(MEANING-PROCEDURES NIL)", FormatToLISP().meaning_procedures_to_lisp("NIL"))
        self.assertEqual("(MEANING-PROCEDURES NIL)", FormatToLISP().meaning_procedures_to_lisp([]))
        self.assertEqual("(MEANING-PROCEDURES (MPNAME))", FormatToLISP().meaning_procedures_to_lisp([["MPNAME"]]))
        self.assertEqual("(MEANING-PROCEDURES (MPNAME ARG1 2))", FormatToLISP().meaning_procedures_to_lisp([["MPNAME", "ARG1", 2]]))
        self.assertEqual("(MEANING-PROCEDURES (MPNAME ARG1 (REC1 2)))", FormatToLISP().meaning_procedures_to_lisp([["MPNAME", "ARG1", ["REC1", 2]]]))
        self.assertEqual("(MEANING-PROCEDURES (MPNAME ARG1) (MP2 ARG2))", FormatToLISP().meaning_procedures_to_lisp([["MPNAME", "ARG1"], ["MP2", "ARG2"]]))
        self.assertEqual("(MEANING-PROCEDURES (MPNAME (VALUE ^$VAR0)))", FormatToLISP().meaning_procedures_to_lisp([["MPNAME", "^$VAR0"]]))
        self.assertEqual("(MEANING-PROCEDURES (MPNAME (REC1 (VALUE REFSEM4))))", FormatToLISP().meaning_procedures_to_lisp([["MPNAME", ["REC1", "REFSEM4"]]]))
    ...
    def test_example_bindings_to_lisp(self):
        ...
        self.assertEqual("(EXAMPLE-BINDINGS NIL)", FormatToLISP().example_bindings_to_lisp(""))
        self.assertEqual("(EXAMPLE-BINDINGS NIL)", FormatToLISP().example_bindings_to_lisp("NIL"))
        self.assertEqual("(EXAMPLE-BINDINGS NIL)", FormatToLISP().example_bindings_to_lisp([]))
        self.assertEqual("(EXAMPLE-BINDINGS (A B 1 2))", FormatToLISP().example_bindings_to_lisp(["A", "B", 1, 2]))
        self.assertEqual("(EXAMPLE-BINDINGS (A B 1 2) (C D 3 4))", FormatToLISP().example_bindings_to_lisp([["A", "B", 1, 2], ["C", "D", 3, 4]]))
    ...
    def test_example_deps_to_lisp(self):
        ...
        self.assertEqual("(EXAMPLE-DEPS NIL)", FormatToLISP().example_deps_to_lisp("NIL"))
        self.assertEqual("(EXAMPLE-DEPS NIL)", FormatToLISP().example_deps_to_lisp([]))
        self.assertEqual("(EXAMPLE-DEPS ((TYPE A B) (XYZ 1 2)))", FormatToLISP().example_deps_to_lisp([["TYPE", "A", "B"], ["XYZ", 1, 2]]))
    ...
    def test_synonyms_to_lisp(self):
        ...
        self.assertEqual("(SYNONYMS (WORDA))", FormatToLISP().synonyms_to_lisp(["WORDA"]))
        self.assertEqual("(SYNONYMS (WORDA WORDB))", FormatToLISP().synonyms_to_lisp(["WORDA", "WORDB"]))
        ...
        self.assertEqual("(SYNONYMS (\"X123\"))", FormatToLISP().synonyms_to_lisp(["X123"]))
    ...
    def test_hyponyms_to_lisp(self):
        ...
        self.assertEqual("(HYPONYMS (WORDA))", FormatToLISP().hyponyms_to_lisp(["WORDA"]))
        self.assertEqual("(HYPONYMS (WORDA WORDB))", FormatToLISP().hyponyms_to_lisp(["WORDA", "WORDB"]))
        ...
        self.assertEqual("(HYPONYMS (\"X123\"))", FormatToLISP().hyponyms_to_lisp(["X123"]))
    ...
    def test_use_with_types_to_lisp(self):
        ...
        self.assertEqual("(USE-WITH-TYPES NIL)", FormatToLISP().use_with_types_to_lisp(""))
        self.assertEqual("(USE-WITH-TYPES NIL)", FormatToLISP().use_with_types_to_lisp("NIL"))
        self.assertEqual("(USE-WITH-TYPES NIL)", FormatToLISP().use_with_types_to_lisp([]))
        self.assertEqual("(USE-WITH-TYPES (T1))", FormatToLISP().use_with_types_to_lisp(["T1"]))
        self.assertEqual("(USE-WITH-TYPES (T1 T2))", FormatToLISP().use_with_types_to_lisp(["T1", "T2"]))
...
class ExampleTestCase(TestCase):
    ...
    def test_specific_examples(self):
        ...
        # YTTRIUM-N1
        self.assertEqual("(SEM-STRUC (YTTRIUM))", FormatToLISP().semstruc_to_lisp("YTTRIUM"))
        # WORSEN-V3
        self.assertEqual("(MEANING-PROCEDURES (FIX-CASE-ROLE (VALUE ^$VAR1) (VALUE ^$VAR0)))", FormatToLISP().meaning_procedures_to_lisp([
            [
              "FIX-CASE-ROLE",
              "^$VAR1",
              "^$VAR0"
        ...
        # YOURSELVES-N2
        self.assertEqual("(SEM-STRUC (MODALITY (TYPE SALIENCY) (VALUE 1) (SCOPE (VALUE ^$VAR1))))", FormatToLISP().semstruc_to_lisp({
            "MODALITY": {
              "TYPE": "SALIENCY",
              "VALUE": 1,
              "SCOPE": {
        ...
        # WRINKLE-V2
        self.assertEqual("(SEM-STRUC (CHANGE-EVENT (THEME (VALUE ^$VAR1) (SEM (PAPER SEWING-MATERIAL))) (PRECONDITION (VALUE REFSEM1)) (EFFECT (VALUE REFSEM2))) (REFSEM1 (SMOOTHNESS (DOMAIN (VALUE ^$VAR1)))) (REFSEM2 (SMOOTHNESS (DOMAIN (VALUE ^$VAR1)))) (> (VALUE REFSEM1.RANGE) (VALUE REFSEM2.RANGE)))", FormatToLISP().semstruc_to_lisp({
            "CHANGE-EVENT": {
                "THEME": {
                    "VALUE": "^$VAR1",
                    "SEM": [
        ...
        # YOURSELF-N2
        self.assertEqual("(SYN-STRUC ((N ((ROOT $VAR1) (CAT N) (ROOT YOU))) (N ((ROOT $VAR0) (CAT N) (TYPE REFL-PRO)))))", FormatToLISP().synstruc_to_lisp(OrderedDict({
            "N": {
                "ROOT": "$VAR1",
                "CAT": "N",
                "ROOT-WORD": "YOU"
        ...
        # BABY'S_ROOM-N1
        self.assertEqual("(SEM-STRUC (BEDROOM (LOCATION-OF CRIB) (LOCATION-OF (VALUE REFSEM1))) (REFSEM1 (SLEEP (EXPERIENCER (VALUE REFSEM2)))) (REFSEM2 (HUMAN (AGE (< 3)))))", FormatToLISP().semstruc_to_lisp({
            "BEDROOM": {
                "LOCATION-OF": "CRIB",
                "LOCATION-OF-1": {
                    "VALUE": "REFSEM1"
        ...
        # ABOUT-PREP4
        self.assertEqual("(SYN-STRUC ((ROOT $VAR1) (CAT (V N)) (PP ((ROOT $VAR0) (CAT PREP) (OBJ ((ROOT $VAR2) (CAT NP)))))))", FormatToLISP().synstruc_to_lisp(OrderedDict({
            "ROOT": "$VAR1",
            "CAT": [
                "V",
                "N"
        ...
        # AND-CONJ20
        self.assertEqual("(OUTPUT-SYNTAX (N V))", FormatToLISP().output_syntax_to_lisp([
            "N",
            "V"
        ]))
        # FOR_EXAMPLE-CONJ1
        self.assertEqual("(EXAMPLE-BINDINGS (THE OBJECT-1 *COMMA*-3 FOR_EXAMPLE-0 *COMMA*-4 THE EVENT-2 *COMMA*-5))", FormatToLISP().example_bindings_to_lisp([
            "THE",
            "OBJECT-1",
            "*COMMA*-3",
            "FOR_EXAMPLE-0",
        ...
        # VALENTINE'S_DAY-N1
        self.assertEqual("(SYNONYMS (\"SAINT_VALENTINE'S_DAY\" \"ST._VALENTINE'S_DAY\"))", FormatToLISP().synonyms_to_lisp([
            "SAINT_VALENTINE'S_DAY",
            "ST._VALENTINE'S_DAY"
        ]))
        # THUNDER-V1
        self.assertEqual("(SEM-STRUC (THUNDER) (^$VAR1 (NULL-SEM +)))", FormatToLISP().semstruc_to_lisp({
            "THUNDER": {},
            "^$VAR1": {
                "NULL-SEM": "+"
            }
        ...
        self.assertEqual("(SEM-STRUC NIL)", FormatToLISP().semstruc_to_lisp(""))
        # *PERIOD*-PUNCT1
        self.assertEqual("*PERIOD*-PUNCT1", FormatToLISP()._escape_sense_name("*PERIOD*-PUNCT1"))
        # 16_INCH_GUN-N1
        self.assertEqual("\"16_INCH_GUN-N1\"", FormatToLISP()._escape_sense_name("16_INCH_GUN-N1"))
...
class FormatFromLISPTestCase(TestCase):
    ...
    def test_lisp_to_sense(self):
        ...
        formatter.list_to_sense.assert_called_once_with([["A", ["B", "C"]], ["D", "E"], "F"])
    ...
    def test_list_to_sense(self):
        ...
        formatter.parse_comments = MagicMock(side_effect=lambda x: "COMMENTS-PARSED-%s" % x[1])
        formatter.parse_tmr_head = MagicMock(side_effect=lambda x: "TMR-HEAD-PARSED-%s" % x[1])
        formatter.parse_syn_struc = MagicMock(side_effect=lambda x: "SYN-STRUC-PARSED-%s" % x[1])
        formatter.parse_sem_struc = MagicMock(side_effect=lambda x: "SEM-STRUC-PARSED-%s" % x[1])
        formatter.parse_output_syntax = MagicMock(side_effect=lambda x: "OUTPUT-SYNTAX-PARSED-%s" % x[1])
        formatter.parse_meaning_procedures = MagicMock(side_effect=lambda x: "MEANING-PROCEDURES-PARSED-%s" % x[1])
        formatter.parse_example_bindings = MagicMock(side_effect=lambda x: "EXAMPLE-BINDINGS-PARSED-%s" % x[1])
        formatter.parse_example_deps = MagicMock(side_effect=lambda x: "EXAMPLE-DEPS-PARSED-%s" % x[1])
        formatter.parse_synonyms = MagicMock(side_effect=lambda x: "SYNONYMS-PARSED-%s" % x[1])
        formatter.parse_hyponyms = MagicMock(side_effect=lambda x: "HYPONYMS-PARSED-%s" % x[1])
        formatter.parse_types = MagicMock(side_effect=lambda x: "TYPES-PARSED-%s" % x[1])
        formatter.parse_use_with_types = MagicMock(side_effect=lambda x: "USE-WITH-TYPES-PARSED-%s" % x[1])
    ...
    def test_list_key_to_value(self):
        ...
        self.assertEqual(["F", "DEFAULT"], formatter.list_key_to_value(input, "F", "DEFAULT"))
    ...
    def test_parse_cat(self):
        ...
        self.assertEqual(["N", "PREP"], FormatFromLISP().parse_cat(["CAT", ["N", "PREP"]]))

    def test_parse_def(self):
        # DEF can only be a single string

        self.assertEqual("A definition.", FormatFromLISP().parse_def(["DEF", "A definition."]))

    def test_parse_ex(self):
        # EX can only be a single string

        self.assertEqual("An example.", FormatFromLISP().parse_def(["EX", "An example."]))

    def test_parse_comments(self):
        # COMMENTS can only be a single string

        self.assertEqual("Some comments.", FormatFromLISP().parse_def(["COMMENTS", "Some comments."]))
    ...
    def test_parse_tmr_head(self):
        ...
        self.assertEqual("$VAR1", FormatFromLISP().parse_tmr_head(["TMR-HEAD", "$VAR1"]))
    ...
    def test_parse_syn_struc(self):
        ...
        self.assertEqual({
            "ROOT": "$VAR0",
            "CAT": "V"
        }, FormatFromLISP().parse_syn_struc(["SYN-STRUC", [["ROOT", "$VAR0"], ["CAT", "V"]]]))
        ...
        }, FormatFromLISP().parse_syn_struc(["SYN-STRUC", [["SUBJECT", [["ROOT", "$VAR2"], ["CAT", "NP"]]]]]))
        ...
        }, FormatFromLISP().parse_syn_struc(["SYN-STRUC", [["SUBJECT", [["ROOT", [["X", "ABC"]]], ["CAT", ["D", "E", "F"]]]]]]))

    def test_parse_sem_struc(self):
        # SEM-STRUC a recursive list of strings.

        # (SEM-STRUC (OBJECT))
        self.assertEqual("OBJECT", FormatFromLISP().parse_sem_struc(["SEM-STRUC", ["OBJECT"]]))
        ...
        }, FormatFromLISP().parse_sem_struc(["SEM-STRUC", ["ACQUIRE", ["AGENT", ["VALUE", "^$VAR2"]], ["THEME", ["VALUE", "^$VAR3"]]]]))
        ...
        }, FormatFromLISP().parse_sem_struc(["SEM-STRUC", ["HUMAN", ["AGENT-OF", ["VALUE", "REFSEM1"]]], ["REFSEM1", ["ACQUIRE", ["THEME", ["VALUE", "^$VAR2"]]]]]))
        ...
        }, FormatFromLISP().parse_sem_struc(["SEM-STRUC", ["HUMAN", ["NOVELTY", [">", "0.7"]]]]))
        ...
        }, FormatFromLISP().parse_sem_struc(["SEM-STRUC", ["DOG"], ["^$VAR0", ["RELATION", ["VALUE", "^$VAR100"]]]]))
    ...
    def test_parse_output_syntax(self):
        ...
        self.assertEqual("NIL", FormatFromLISP().parse_output_syntax(["OUTPUT-SYNTAX", "NIL"]))
        self.assertEqual("INF-CL", FormatFromLISP().parse_output_syntax(["OUTPUT-SYNTAX", "INF-CL"]))
        self.assertEqual(["N", "V"], FormatFromLISP().parse_output_syntax(["OUTPUT-SYNTAX", ["N", "V"]]))
        self.assertEqual("NIL", FormatFromLISP().parse_output_syntax(["OUTPUT-SYNTAX", []]))
    ...
    def test_parse_meaning_procedures(self):
        ...
        self.assertEqual("NIL", FormatFromLISP().parse_meaning_procedures(["MEANING-PROCEDURES", "NIL"]))
        self.assertEqual("NIL", FormatFromLISP().parse_meaning_procedures(["MEANING-PROCEDURES", []]))
        self.assertEqual([["APPLY-MEANING", "^$VAR1", "^$VAR2"]], FormatFromLISP().parse_meaning_procedures(["MEANING-PROCEDURES", ["APPLY-MEANING", "^$VAR1", "^$VAR2"]]))
    ...
    def test_parse_example_bindings(self):
        ...
        self.assertEqual("NIL", FormatFromLISP().parse_example_bindings(["EXAMPLE-BINDINGS", "NIL"]))
        self.assertEqual("NIL", FormatFromLISP().parse_example_bindings(["EXAMPLE-BINDINGS", []]))
        self.assertEqual(["THE", "MAN", "HIT-1"], FormatFromLISP().parse_example_bindings(["EXAMPLE-BINDINGS", ["THE", "MAN", "HIT-1"]]))
    ...
    def test_parse_example_deps(self):
        ...
        self.assertEqual("NIL", FormatFromLISP().parse_example_deps(["EXAMPLE-DEPS", "NIL"]))
        self.assertEqual("NIL", FormatFromLISP().parse_example_deps(["EXAMPLE-DEPS", []]))
        self.assertEqual([["NSUBJ", "$VAR0", "$VAR1"], ["PREP", "$VAR0", "$VAR3"]], FormatFromLISP().parse_example_deps(["EXAMPLE-DEPS", [["NSUBJ", "$VAR0", "$VAR1"], ["PREP", "$VAR0", "$VAR3"]]]))
    ...
    def test_parse_synonyms(self):
        ...
        self.assertEqual(["A", "B"], FormatFromLISP().parse_synonyms(["SYNONYMS", ["A", "B"]]))
    ...
    def test_parse_hyponyms(self):
        ...
        self.assertEqual(["A", "B"], FormatFromLISP().parse_hyponyms(["HYPONYMS", ["A", "B"]]))
    ...
    def test_parse_types(self):
        ...
        self.assertEqual(["A", "B"], FormatFromLISP().parse_types(["TYPES", ["A", "B"]]))

    def test_parse_use_with_types(self):
        # USE-WITH-TYPES is either NIL or a list of type strings; we also handle an empty list

        self.assertEqual([], FormatFromLISP().parse_use_with_types(["USE-WITH-TYPES", []]))
        self.assertEqual([], FormatFromLISP().parse_use_with_types(["USE-WITH-TYPES", "NIL"]))
        self.assertEqual(["A", "B"], FormatFromLISP().parse_use_with_types(["USE-WITH-TYPES", ["A", "B"]]))